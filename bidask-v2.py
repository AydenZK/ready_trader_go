...
class AutoTrader(BaseAutoTrader):
    ...
    def get_theo(self, bid_prices, bid_volumes, ask_prices, ask_volumes) -> float:
        ...
        if bid_volumes[0]+ask_volumes[0]>0:
            return (bid_prices[0]*bid_volumes[0]+ask_prices[0]*ask_volumes[0])/(bid_volumes[0]+ask_volumes[0])
        else:
    ...
    def get_edges(self, theo, position) -> tuple[int, int]:
        ...
        if len(self.historical.historical_prices[Instrument.ETF])<=5:
            bid_edge = 5000*st.norm.ppf(buy_prob)
            ask_edge = 5000*st.norm.ppf(1-sell_prob)
        else:
