...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.second_timer = UnhedgedTimer(event_loop=self.event_loop)
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE and self.safe_to_trade: ## mm order:
            ...
            if bid_order and self.bid_id == 0 and bid_order.price != 0 and self.potential_position.max + bid_order.vol <= POSITION_LIMIT:
                ...
                self.custom_log({"ACTION": f"BUY {bid_order.vol} ETF @{bid_order.price}, ID: {self.bid_id}, type: {order_type}, edge: {bid_order.edge}"})
            ...
            if ask_order and self.ask_id == 0 and ask_order.price != 0 and self.potential_position.min-ask_order.vol >= -POSITION_LIMIT:
                ...
                self.custom_log({"ACTION": f"SELL {ask_order.vol} ETF @{ask_order.price}, ID: {self.ask_id}, type: {order_type}, edge: {ask_order.edge}"})
    ...
    def custom_hedge(self):
        leniency = 5
        next_id = next(self.order_ids)
        if self.position + self.futures_position > 10:
            trade_vol = self.position + self.futures_position + leniency
            self.send_hedge_order(client_order_id=next_id, side=Side.SELL, price=MINIMUM_BID, volume=trade_vol) # selling futures (mkt order)
    ...
    def get_volumes(self):
        ...
        lot_step = 5
        mm_limit = max(abs(self.futures_position), MM_LIMIT)
        if self.position < -mm_limit:
            return (LOT_SIZE+lot_step, LOT_SIZE-lot_step)
        elif self.position > mm_limit:
            return (LOT_SIZE-lot_step, LOT_SIZE+lot_step)
        else:
            if self.position >= mm_limit - LOT_SIZE:
                return (mm_limit-self.position, LOT_SIZE+lot_step)
            if self.position <= -mm_limit + LOT_SIZE:
                return (LOT_SIZE+lot_step, mm_limit+self.position)
            return (LOT_SIZE, LOT_SIZE)
